//! TipTap Text Opacity Manager - Aerospace-Grade Text Opacity Operations Service
//!
//! Safety-critical text opacity operations service with:
...
pub struct TextOpacityManager {
    config_service: Arc<ExportConfigService>,
    operation_count: u64,
    last_error: Option<ErrorContext>,
}

impl TextOpacityManager {
    /// Creates a new text opacity manager instance
    /// 
    /// # Arguments
    /// * `config_service` - The configuration service
    /// 
    /// # Returns
    /// A new TextOpacityManager instance
    ...
    pub fn apply_opacity(&mut self, node: &mut TipTapNode, opacity: f64) -> Result<(), String> {
        ...
        if let Some(ref mut attrs) = node.attrs {
            if let Some(obj) = attrs.as_object_mut() {
                obj.insert("textOpacity".to_string(), serde_json::json!(opacity));
            }
        } else {
            node.attrs = Some(serde_json::json!({ "textOpacity": opacity }));
        }

        // Performance monitoring
        let elapsed = start_time.elapsed();
        if elapsed.as_millis() > PERFORMANCE_CRITICAL_THRESHOLD_MS {
            eprintln!("Text opacity application CRITICAL performance warning: took {}ms", elapsed.as_millis());
        } else if elapsed.as_millis() > PERFORMANCE_WARNING_THRESHOLD_MS {
            eprintln!("Text opacity application performance warning: took {}ms", elapsed.as_millis());
        }
    ...
    /// Remove text opacity from a node
    /// 
    /// # Arguments
    ...
    pub fn remove_opacity(&mut self, node: &mut TipTapNode) -> Result<(), String> {
        ...
        let elapsed = start_time.elapsed();
        if elapsed.as_millis() > PERFORMANCE_CRITICAL_THRESHOLD_MS {
            eprintln!("Text opacity removal CRITICAL performance warning: took {}ms", elapsed.as_millis());
        } else if elapsed.as_millis() > PERFORMANCE_WARNING_THRESHOLD_MS {
            eprintln!("Text opacity removal performance warning: took {}ms", elapsed.as_millis());
        }
    ...
    pub fn get_opacity(&self, node: &TipTapNode) -> Option<f64> {
        if let Some(ref attrs) = node.attrs {
            if let Some(obj) = attrs.as_object() {
                if let Some(opacity) = obj.get("textOpacity") {
    ...
    /// Check if node has text opacity
    /// 
    /// # Arguments
    /// * `node` - The node to check
    /// 
    /// # Returns
    /// True if node has text opacity, false otherwise
...
mod tests {
    ...
    #[test]
    fn test_text_opacity_manager_creation() {
        let config_service = Arc::new(ExportConfigService::new());
        let manager = TextOpacityManager::new(config_service);
        assert_eq!(manager.get_operation_count(), 0);
    }

    #[test]
    fn test_performance_threshold_getters() {
        assert_eq!(TextOpacityManager::performance_warning_threshold_ms(), PERFORMANCE_WARNING_THRESHOLD_MS);
        assert_eq!(TextOpacityManager::performance_critical_threshold_ms(), PERFORMANCE_CRITICAL_THRESHOLD_MS);
    }

    #[test]
    fn test_min_max_constants() {
        assert_eq!(TextOpacityManager::min_opacity(), MIN_OPACITY);
        assert_eq!(TextOpacityManager::max_opacity(), MAX_OPACITY);
    }

    #[test]
    fn test_apply_opacity() {
        let config_service = Arc::new(ExportConfigService::new());
        let mut manager = TextOpacityManager::new(config_service);
    ...
    fn test_remove_opacity() {
        ...
        let mut node = TipTapNode {
            ...
            content: None,
            text: Some("Test".to_string()),
            attrs: Some(serde_json::json!({ "textOpacity": 0.5 })),
            marks: None,
        };
    ...
    fn test_get_opacity() {
        ...
        manager.apply_opacity(&mut node, 0.75).unwrap();
    ...
    fn test_has_opacity() {
        ...
        manager.apply_opacity(&mut node_with, 0.5).unwrap();
